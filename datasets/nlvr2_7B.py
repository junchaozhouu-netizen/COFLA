...
class NLVR2Dataset(BaseMultimodalDataset):
    """7B-only NLVR2 dataset with robust image validation.

    It mirrors datasets/nlvr2.py, but skips samples whose left/right image is
    missing, unreadable, or has non-positive dimensions before Qwen2-VL sees them.
    """
    ...
    def __init__(
        ...
        for row in rows:
            ...
            left_ok, left_reason = validate_image_value(left_image, root=root)
            right_ok, right_reason = validate_image_value(right_image, root=root)
            if not left_ok or not right_ok:
                self.invalid_image_records.append(
                    {
                        "id": sample_id,
    ...
    def summary(self) -> Dict[str, Any]:
        return {
            ...
            "num_invalid_image_skipped": len(self.invalid_image_records),
        }
...
class NLVR2Collator:
    ...
    def __call__(self, batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        dropped = []
        for sample in batch:
            left_image = safe_coerce_image_to_pil(sample["left_image"], root=self.data_root)
            right_image = safe_coerce_image_to_pil(sample["right_image"], root=self.data_root)
            if left_image is None:
                left_image = qwen2vl_blank_image()
            if right_image is None:
                right_image = qwen2vl_blank_image()
        ...
        if not sample_ids:
            raise RuntimeError(f"All NLVR2 samples in this 7B batch have invalid images. dropped_ids={dropped}")
