...
class InternVL3Wrapper(nn.Module):
    def __init__(self, args):
        ...
        self.model_name = str(getattr(args, "model_name", "internvl3_1b")).lower()
        self.model_family = str(getattr(args, "model_family", "internvl")).lower()
        ...
        if self.model_type != "internvl_chat":
            raise ValueError(
                f"InternVL3Wrapper expected model_type='internvl_chat', but got {self.model_type!r} from {args.model_path}"
            )
        ...
        self.image_size = int(getattr(self.hf_config, "force_image_size", 448) or 448)
        self.max_dynamic_tiles = int(getattr(args, "internvl_max_dynamic_tiles", 12))
        self.use_thumbnail = bool(getattr(args, "internvl_use_thumbnail", True))
        self.image_transform = self._build_image_transform(self.image_size)
        ...
        requested_flash_attn = bool(getattr(args, "use_flash_attn", not bool(getattr(args, "deterministic", True))))
        if getattr(args, "deterministic", True):
            requested_flash_attn = False
        model_load_kwargs["use_flash_attn"] = requested_flash_attn
        ...
        self.model = self._load_model(args.model_path, model_load_kwargs)
        self._enable_model_gradient_checkpointing()

        self.config = self.model.config
        self.num_image_token = int(getattr(self.model, "num_image_token", getattr(self.hf_config, "num_image_token", 256)))
        self.img_context_token_id = int(self.tokenizer.convert_tokens_to_ids(IMG_CONTEXT_TOKEN))
        if self.img_context_token_id < 0:
            raise RuntimeError(
                f"Tokenizer for {args.model_path} does not contain {IMG_CONTEXT_TOKEN}. "
                "Check the local tokenizer files in the InternVL3 checkpoint."
            )
        self._set_img_context_token_id()
        self.special_token_ids = tuple(
            sorted({int(token_id) for token_id in getattr(self.tokenizer, "all_special_ids", []) if token_id is not None})
        )
        self.text_proxy_excluded_token_ids = tuple(token_id for token_id in self.special_token_ids if token_id != self.img_context_token_id)
        self.conv_template = copy.deepcopy(getattr(self.model, "conv_template", None))
        self.template_name = str(getattr(self.model, "template", "internvl2_5"))
        self.system_message = str(getattr(self.model, "system_message", ""))
    ...
    def _load_model(self, model_path: str, model_load_kwargs: Dict[str, Any]):
        try:
            return AutoModel.from_pretrained(model_path, **model_load_kwargs)
        except (TypeError, ImportError, ModuleNotFoundError) as exc:
            retry_kwargs = dict(model_load_kwargs)
            flash_related = "flash" in str(exc).lower() or "attn" in str(exc).lower()
            if "use_flash_attn" in retry_kwargs:
                retry_kwargs.pop("use_flash_attn", None)
                if flash_related:
                    warnings.warn(
                        "InternVL3 model loading fell back after a flash-attention related error. "
                        "Continuing without explicit use_flash_attn.",
                        RuntimeWarning,
                    )
                return AutoModel.from_pretrained(model_path, **retry_kwargs)
            raise

    def _set_img_context_token_id(self) -> None:
        chat_model = self._get_chat_model()
        if chat_model is not None:
            setattr(chat_model, "img_context_token_id", self.img_context_token_id)
    ...
    def _get_chat_model(self):
        ...
        while candidates:
            ...
            if oid in seen:
                continue
            seen.add(oid)
            if hasattr(obj, "vision_model") and hasattr(obj, "language_model") and hasattr(obj, "mlp1"):
                return obj
    ...
    def _enable_model_gradient_checkpointing(self) -> None:
        if not getattr(self.args, "gradient_checkpointing", False):
            return
        targets = [self.model, self._get_chat_model(), getattr(self._get_chat_model(), "language_model", None)]
    ...
    def _resolve_hidden_size(self) -> int:
        ...
        if callable(to_dict):
            ...
            if config_dict.get("hidden_size") is not None:
                return int(config_dict["hidden_size"])
        return 896
    ...
    def _resolve_multiclass_label_words(self, words: List[str]) -> LabelWordInfo:
        ...
        for word in words:
            ...
            if found is None:
                raise RuntimeError(
                    f"Could not tokenize label word {word!r} into a single token for InternVL3. "
                    "Please override --label_words with tokenizer-compatible single-token labels."
                )
            text, token_id = found
            texts.append(text)
            token_ids.append(token_id)
        return LabelWordInfo(texts=tuple(texts), token_ids=tuple(token_ids), used_fallback=fallback_used)
    ...
    def describe_trainable_state(self, preview_limit: Optional[int] = None) -> Dict[str, Any]:
        report = collect_trainable_parameter_report(
            ...
            lm_layer_boundary=None if ("matched_lora" in str(getattr(self.scope_config, "scope_name", "")) or "branch_lora" in str(getattr(self.scope_config, "scope_name", ""))) else (self.scope_config.num_lm_layers if self.scope_config.num_lm_layers > 0 else 0),
        )
    ...
    def describe_runtime(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "proxy_definition": {
                "visual_proxy": "Layer-0 hidden states restricted to <IMG_CONTEXT> token positions after mlp1 projection.",
                "text_proxy": "Layer-0 hidden states restricted to non-image text token positions after excluding specials and supervision labels.",
                "fusion_path": "InternVL3 projector mlp1 plus the configured early language_model layers.",
            },
        }
    ...
    @staticmethod
    def _build_image_transform(input_size: int):
        return Compose(
            [
                Lambda(lambda img: img.convert("RGB") if getattr(img, "mode", "RGB") != "RGB" else img),
    ...
    def _dynamic_preprocess(self, image, min_num: int = 1) -> List[Any]:
        orig_width, orig_height = image.size
        aspect_ratio = orig_width / orig_height
        target_ratios = sorted(
            {
                (i, j)
                for n in range(min_num, self.max_dynamic_tiles + 1)
                for i in range(1, n + 1)
                for j in range(1, n + 1)
                if min_num <= i * j <= self.max_dynamic_tiles
            },
        ...
        for block_idx in range(blocks):
            box = (
                (block_idx % (target_width // self.image_size)) * self.image_size,
                (block_idx // (target_width // self.image_size)) * self.image_size,
                ((block_idx % (target_width // self.image_size)) + 1) * self.image_size,
                ((block_idx // (target_width // self.image_size)) + 1) * self.image_size,
            )
    ...
    def _encode_image_tiles(self, image) -> Tuple[torch.Tensor, int]:
        processed_images = self._dynamic_preprocess(image)
        pixel_values = [self.image_transform(tile) for tile in processed_images]
        return torch.stack(pixel_values), len(processed_images)

    def _build_query(self, question: str, num_patches_list: Sequence[int]) -> str:
        question = str(question or "").strip()
        if num_patches_list and "<image>" not in question:
            if len(num_patches_list) == 1:
                question = "<image>\n" + question
            else:
                numbered = [f"Image-{idx + 1}: <image>" for idx in range(len(num_patches_list))]
                question = "\n".join(numbered + [question])

        if self.conv_template is None:
            raise RuntimeError(
                "InternVL3 conversation template is unavailable on the loaded model. "
                "Check the local custom-code files in the checkpoint."
            )
    ...
    def _encode_single_prompt(
        ...
        flag_tensors: List[torch.Tensor] = []

        for image in actual_images:
            image_tiles, num_patches = self._encode_image_tiles(image)
            pixel_tensors.append(image_tiles)
            flag_tensors.append(torch.ones((num_patches, 1), dtype=torch.long))
            num_patches_list.append(int(num_patches))

        if not pixel_tensors:
            pixel_tensors.append(torch.zeros((1, 3, self.image_size, self.image_size), dtype=torch.float32))
            flag_tensors.append(torch.zeros((1, 1), dtype=torch.long))

        query = self._build_query(question, num_patches_list)
        effective_truncation = bool(truncation) and not num_patches_list
        kwargs = {
            "text": query,
            "padding": False,
        ...
        model_inputs = self.tokenizer(**kwargs)
        model_inputs["pixel_values"] = torch.cat(pixel_tensors, dim=0)
        model_inputs["image_flags"] = torch.cat(flag_tensors, dim=0)
        return model_inputs

    def _build_single_token_full_inputs(
        self,
    ...
    def prepare_nlvr2_batch(
        ...
        for sample_id, statement, label, image_pair in zip(sample_ids, statements, labels, image_pairs):
            ...
            prompt_text = "Image-1: <image>\nImage-2: <image>\n" + nlvr2_yes_no_prompt(statement)
            sample_specs.append(
                {
                    "id": sample_id,
                    "label": int(label),
    ...
    def prepare_scienceqa_batch(
        ...
        for sample_id, question, option_list, label, hint, image in zip(sample_ids, questions, choices, labels, hints, images):
            ...
            prompt_text = scienceqa_multichoice_prompt(
                question=question,
                choices=option_list,
    ...
    def _get_hook_target(self):
        ...
        raise RuntimeError("Could not locate the InternVL3 decoder model for registering layer-0 hooks.")
    ...
    def _move_any_to_device(self, value, device):
        ...
        if isinstance(value, Mapping):
            return {key: self._move_any_to_device(val, device) for key, val in value.items()}
        if isinstance(value, list):
            return [self._move_any_to_device(item, device) for item in value]
        if isinstance(value, tuple):
            return tuple(self._move_any_to_device(item, device) for item in value)
        return value

    def move_batch_to_device(self, batch: Dict[str, Any]) -> Dict[str, Any]:
        return {key: self._move_any_to_device(value, self.device_obj) for key, value in batch.items()}
    ...
    def _filter_forward_kwargs(self, target, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if removed:
            self._warn_geometry_once(
                ...
                "InternVL3 forward does not accept some generic wrapper kwargs; "
    ...
    def _call_model(self, inputs: Dict[str, torch.Tensor], labels: Optional[torch.Tensor] = None, **extra_kwargs):
        ...
        if "pixel_values" in kwargs and self.device_obj.type == "cuda":
            kwargs["pixel_values"] = kwargs["pixel_values"].to(dtype=self.compute_dtype)
        ...
        try:
            with self.get_autocast_context():
                ...
                if hidden_states is None:
                    raise RuntimeError("InternVL3 did not return hidden states for task-head classification.")
                ...
                if labels is not None:
                    loss = F.binary_cross_entropy_with_logits(
                        logits.float(),
                        labels.to(logits.device, dtype=torch.float32),
                    )
        finally:
            hook.remove()
            if param_perturb:
    ...
    def compute_prompt_scores(self, batch: Dict[str, Any]) -> Dict[str, torch.Tensor]:
        if batch["dataset_name"] == "mmimdb":
            ...
            return {"probs": probs, "scores": probs, "preds": preds}
        ...
        if batch["dataset_name"] in {"hateful_memes", "nlvr2"}:
            ...
            return {"probs": probs, "scores": pair_probs, "preds": preds}
    ...
    def _build_branch_masks(self, batch: Dict[str, Any], hidden: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        image_token_mask = (input_ids == self.img_context_token_id) & attention_mask
        ...
        # For InternVL3 the layer-0 decoder states are already post-projector mixed-token states.
        # These masks are practical proxy regions rather than the idealized unimodal paper branches.
        text_token_mask = attention_mask & (~image_token_mask) & (~special_token_mask) & (~supervision_mask)
        if not bool(text_token_mask.any()):
            self._warn_geometry_once(
                "empty_text_proxy",
                "InternVL3 text proxy mask is empty after excluding image/special/supervision tokens; text-side geometry will default to zero for this batch.",
            )
    ...
    def build_param_perturb(
        ...
        scale = radius / (norm + eps)
        perturb = {
            name: grad.to(dtype=param.dtype, device=param.device) * scale
            for (name, param), grad in ((item, grads[item[0]]) for item in named_params if item[0] in grads)
        }
    ...
    def compute_geometry_probe(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if hidden is None:
            raise RuntimeError("Failed to capture InternVL3 layer-0 hidden states for geometry computation.")
        ...
        if hidden_grad is None:
            raise RuntimeError("Failed to compute gradients w.r.t. InternVL3 captured layer-0 hidden states.")

        image_mask, text_mask = self._build_branch_masks(batch, hidden)
        if not bool(image_mask.any()):
            raise RuntimeError(
                "InternVL3 geometry probe found no <IMG_CONTEXT> tokens in this batch. "
                "This usually means the sample is text-only or prompt formatting lost the image placeholders."
            )
    ...
    def export_trainable_state(self, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        filtered = {key: value.detach().cpu() for key, value in state_dict.items() if key in trainable_keys or key.startswith("task_head")}
        return {"model_state_dict": filtered, "metadata": dict(metadata or {})}
    ...
    def save_trainable_checkpoint(self, save_dir: str, metadata: Dict[str, Any]) -> None:
        ...
        with open(os.path.join(save_dir, "metadata.json"), "w", encoding="utf-8") as handle:
            json.dump(state["metadata"], handle, ensure_ascii=False, indent=2)

    def load_trainable_checkpoint(self, ckpt_dir: str, strict: bool = False) -> Dict[str, Any]:
        import os

        state = torch.load(os.path.join(ckpt_dir, "trainable_state.pt"), map_location="cpu")
