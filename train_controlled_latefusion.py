...
def _is_dedicated_perturbation_dataset(args) -> bool:
    return str(getattr(args, "dataset", "")).strip().lower() in {"nlvr2", "scienceqa"}


def _geometry_fp32_enabled(args) -> bool:
    """Enable fp32 geometry only for the dedicated NLVR2/ScienceQA entrypoint.

    The original Hateful Memes and MM-IMDb code paths do not define this flag,
    so their autocast behavior remains exactly as before.
    """
    return _is_dedicated_perturbation_dataset(args) and bool(
        getattr(args, "nlvr2_scienceqa_geometry_fp32", False)
    )


def _train_step_autocast_enabled(args) -> bool:
    if (
        _geometry_fp32_enabled(args)
        and str(getattr(args, "method", "")).strip().lower() == "cofla"
    ):
        return False
    return bool(getattr(args, "fp16", False))


def _scienceqa_choice_mask(model: ControlledLateFusionModel, batch: Dict[str, Any]):
    if str(getattr(model, "dataset_type", "")).strip().lower() == "scienceqa":
        return batch.get("choice_mask")
    return None
    ...
    if bool(getattr(args, "auto_rho_f", False)):
        ...
        def _compute_stats(batch):
            with torch.enable_grad():
                with maybe_autocast(device, args.fp16 and not _geometry_fp32_enabled(args)):
...
def compute_fast_train_geometry_from_probe(
    ...
    *,
    ...
    loss_f = model.forward_from_activations(
        ...
        z_t,
        labels=batch["labels"],
        choice_mask=_scienceqa_choice_mask(model, batch),
...
def compute_controlled_per_sample_losses(
    ...
    *,
    ...
    if dataset_key == "hateful_memes":
        # Original Hateful Memes path: unchanged.
    ...
    if dataset_key == "mmimdb":
        # Original MM-IMDb path: unchanged.
    ...
    if dataset_key == "scienceqa":
        ...
        if choice_mask is not None:
            ...
            if valid_choices.shape != logits.shape:
                raise ValueError(
                    "ScienceQA choice_mask shape must match logits for per-sample loss: "
                    f"mask={tuple(valid_choices.shape)}, logits={tuple(logits.shape)}"
                )
            masked_logits = logits.masked_fill(~valid_choices, torch.finfo(logits.dtype).min)
        return F.cross_entropy(masked_logits.float(), targets, reduction="none")

    raise ValueError(f"Unsupported dataset_type for per-sample loss: {dataset_key}")
...
def compute_modality_dropout_losses(
    ...
    *,
    ...
    loss_v = model.forward_from_activations(
        z_v, z_t_zero, labels=labels, choice_mask=choice_mask
    ).loss
    loss_t = model.forward_from_activations(
        z_v_zero, z_t, labels=labels, choice_mask=choice_mask
    ).loss
    loss_empty = model.forward_from_activations(
        z_v_zero, z_t_zero, labels=labels, choice_mask=choice_mask
    ).loss
...
def run_train_step(
    ...
    if method in {"sam", "sam_lora", "esam_lora"}:
        ...
        try:
            ...
            if method == "esam_lora":
                base_per_sample = compute_controlled_per_sample_losses(
                    model,
                    base_outputs.logits,
                    batch["labels"],
                    choice_mask=_scienceqa_choice_mask(model, batch),
                )
                adv_per_sample = compute_controlled_per_sample_losses(
                    model,
                    adv_out.logits,
                    batch["labels"],
                    choice_mask=_scienceqa_choice_mask(model, batch),
    ...
    if method == "masam_lora":
        ...
        loss_v_uni, loss_t_uni, _ = compute_modality_dropout_losses(
            model,
            z_v,
            z_t,
            batch["labels"],
    ...
    if method == "cofla":
        ...
        if cofla_train_geometry == "fast_fcf":
            # COFLA-F: one-probe fusion-exact / branch-calibrated finite-step alignment.
            # The projection objective still uses S_f=[L_f^+-L_0]_+, while the
            # branch denominator is a detached first-order calibration. No EMA
            # directions and no gradient-norm projection are used.
            ...
            fcf_gate = bounded_positive_fcf_gate(geom["rfcf"])
            ...
            # COFLA-E: unchanged exact finite-step alignment.
            ...
            j_bf = torch.relu(geom["rfcf"])
            fcf_gate = bounded_positive_fcf_gate(geom["rfcf"])
            perturbed_loss = (geom["loss_v_plus"] + geom["loss_t_plus"] + geom["loss_f_plus"]) / 3.0
            total_loss = (1.0 - fcf_gate) * base_loss + fcf_gate * perturbed_loss
        ...
        logs["cofla_gate"] = tensor_item(fcf_gate)
    ...
    if method == "msam_lora":
        ...
        base_loss = base_outputs.loss
        z_v, z_t = base_outputs.z_v, base_outputs.z_t
        loss_v_uni, loss_t_uni, loss_empty = compute_modality_dropout_losses(
            model,
            z_v,
            z_t,
            batch["labels"],
    ...
    if method == "dgl_lora":
        ...
        labels = batch["labels"]
        detached_out = model.forward_from_activations(
            z_v.detach(),
            z_t.detach(),
            labels=labels,
            choice_mask=_scienceqa_choice_mask(model, batch),
...
def main() -> None:
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for raw_batch in progress:
            ...
            with maybe_autocast(device, _train_step_autocast_enabled(args)):
